...
[rows, cols] = size(vara);
amean  = sum(vara) / rows;
aavg_mean = sum(aavg) / rows;
asb = sum((amean-aavg_mean).^2)*rows;
amsb = asb / (cols - 1);

astot = zeros(1, cols);
aserr = zeros(1, cols);
for i=1:cols
    astot(i) = sum((vara(:,i) - amean(i)).^2);
end
sumastot = sum(astot);
...
smean  = sum(vars) / rows;
savg_mean = sum(savg) / rows;
ssb = sum((smean-savg_mean).^2)*rows;
smsb = ssb / (cols - 1);

sstot = zeros(1, cols);
for i=1:cols
    sstot(i) = sum((vars(:,i) - smean(i)).^2);
end
sumsstot = sum(sstot);
...
wmean  = sum(varw) / rows;
wavg_mean = sum(wavg) / rows;
wsb = sum((wmean-wavg_mean).^2)*rows;
wmsb = wsb / (cols - 1);

wstot = zeros(1, cols);
for i=1:cols
    wstot(i) = sum((varw(:,i) - wmean(i)).^2);
end
sumwstot = sum(wstot);
