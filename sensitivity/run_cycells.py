...
log_filename = 'status_' + socket.gethostname() + '_' + str(os.getpid()) + '.txt'
...
def getNextRow():
  global current_row, strain, rep
  with(lock):
    while True:
      next_row = current_row
      current_row += 1

      # Skip runs that have already completed and recorded the hist file
      filename = '_'.join([strain_long[strain], rep, str(next_row).zfill(3)])
      if not os.path.isfile('hist/'+filename+'.hist'):
        return next_row
